	...
	uint16_t 				*pOutBuffByteNumbForTx)
{
	...
	*pOutBuffByteNumbForTx = 0u;
	...
	case (IMMPC_ID_AND_PACK_REQUESTS_9dof_main_calib_request_cmd_e):
		...
		{
			*pOutBuffByteNumbForTx =
				(uint16_t) IMMPC_SetMain9dofCalibDataPack(
					pRawSensMeas_s,
					(immpc_9dof_main_calib_pack_s*) pOutBuff);
			/* .... */
		}
		else
		{
 ...
 * 				измерителей
 *
 * @param[in]  	*pInertMeas_s:	Указатель на структуру данных, в которой хранятся 
 * 								измерения всех датчиков и их калибровочные матрицы
 * @param[out]	*pPackForTx_s  	Указатель на область памяти, в которую будет записан пакет данных
 *
 * @return 	Длина пакета данных в байтах
 */
size_t
 ...
 * @date      07-янв-2020
 *
 * @brief    Функция формирует пакет калиброванных данных основных измерителей
 *
 * @param[in]  	*pInertMeas_s:	Указатель на структуру данных, в которой хранятся 
 * 								измерения всех датчиков и их калибровочные матрицы
 * @param[out]	*pPackForTx_s  	Указатель на область памяти, в которую будет записан пакет данных
	...
	immpc_9dof_main_calib_pack_s	*pPackForTx_s)
{
	...
	ISCM_GetCalibDataFromRawGeneric(
		(__ISCM_FPT__*) &pInertMeas_s->main9dof.acc_a[0u],
		(iscm_calibmatrix_generic_s*) &pInertMeas_s->calibMat_s.mainAccCalibMatrix,
		(__ISCM_FPT__*) &pPackForTx_s->acc_a[0u]);

	/* Запись измерений температуры акселерометра */
	pPackForTx_s->accTemp_a[0u] = pInertMeas_s->main9dof.accTemp_a[0u];
	pPackForTx_s->accTemp_a[1u] = pInertMeas_s->main9dof.accTemp_a[0u];
	pPackForTx_s->accTemp_a[2u] = pInertMeas_s->main9dof.accTemp_a[0u];

	/* Запись измерений 3-х осей гироскопа (калиброванные данные) */
	ISCM_GetCalibDataFromRawGeneric(
		pInertMeas_s->main9dof.gyr_a,
		(iscm_calibmatrix_generic_s*) &pInertMeas_s->calibMat_s.mainGyrCalibMatrix,
		pPackForTx_s->gyr_a);

	/* Запись измерений температуры гироскопа */
	pPackForTx_s->gyrTemp_a[0u] = pInertMeas_s->main9dof.gyrTemp_a[0u];
	pPackForTx_s->gyrTemp_a[1u] = pInertMeas_s->main9dof.gyrTemp_a[0u];
	pPackForTx_s->gyrTemp_a[2u] = pInertMeas_s->main9dof.gyrTemp_a[0u];
	...
	return (sizeof(immpc_9dof_main_calib_pack_s));
}
 ...
 * @date		27-ноя-2019
 *
 * @brief    Функция формирует пакет "сырых" данных резервных измерителей
 *
 * @param[in]  	*pInertMeas_s:	Указатель на структуру данных, в которой хранятся 
 * 								измерения всех датчиков и их калибровочные матрицы
 * @param[out]	*pPackForTx_s  	Указатель на область памяти, в которую будет записан пакет данных
 ...
 * @date      07-янв-2020
 *
 * @brief    Функция формирует пакет калиброванных данных резервных измерителей
 *
 * @param[in]  	*pInertMeas_s:	Указатель на структуру данных, в которой хранятся 
 * 								измерения всех датчиков и их калибровочные матрицы
 * @param[out]	*pPackForTx_s  	Указатель на область памяти, в которую будет записан пакет данных
	...
	immpc_9dof_reserve_calib_pack_s	*pPackForTx_s)
{
	...
	/* Запись измерений акселерометра */
	ISCM_GetCalibDataFromRawGeneric(
		(__ISCM_FPT__*) &pInertMeas_s->reserve6dof.acc_a[0u],
		(iscm_calibmatrix_generic_s*) &pInertMeas_s->calibMat_s.reserveAccCalibMatrix,
		(__ISCM_FPT__*) &pPackForTx_s->acc_a[0u]);

	/* Запись измерений температуры акселерометра */
	pPackForTx_s->accTemp_a[0u] = pInertMeas_s->reserve6dof.accTemp_a[0u];
	pPackForTx_s->accTemp_a[1u] = pInertMeas_s->reserve6dof.accTemp_a[0u];
	pPackForTx_s->accTemp_a[2u] = pInertMeas_s->reserve6dof.accTemp_a[0u];
	...
	/* Запись измерений температуры гироскопа */
	pPackForTx_s->gyrTemp_a[0u] = pInertMeas_s->reserve6dof.gyrTemp_a[0u];
	pPackForTx_s->gyrTemp_a[1u] = pInertMeas_s->reserve6dof.gyrTemp_a[0u];
	pPackForTx_s->gyrTemp_a[2u] = pInertMeas_s->reserve6dof.gyrTemp_a[0u];
 ...
 * 				измерителей
 *
 * @param[in]	*pSourceData_s:	Указатель на область памяти, в которой хранятся
 * 								"сырые" данные
 *
